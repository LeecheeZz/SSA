    ...
    for query, reference, ids, labels in bar:
        ...
        if scaler:
            with (autocast()):  # -- 使用混合精度
                ...
                if torch.cuda.device_count() > 1 and len(train_config.gpu_ids) > 1:
                    ...
                    criterion_dkd = DKD(alpha=0.0, beta=1.0, temperature=4)
                    # for University-1652
                    # nc_kl_loss =  min(bar.n / bar.total*0.1, 1.0) * cal_dkd_loss(features_cls_1, features_cls_2, labels, criterion_dkd)
                    # for SUES-200
                    nc_kl_loss = cal_dkd_loss(features_cls_1, features_cls_2, labels, criterion_dkd)

                    # loss_kl = nn.KLDivLoss(reduction='batchmean')
                    # nc_kl_loss = cal_kl_loss(features_cls_1, features_cls_2, loss_kl)
                    # from sample4geo.loss.triplet_loss import Tripletloss,TripletLoss
                    # triplet_loss = Tripletloss(margin=0.3)
                    # f_triplet_loss = cal_triplet_loss(features_tri_1,features_tri_2,labels,triplet_loss,1)
        ...
        if train_config.verbose:
            ...
            monitor = {
                ...
                "loss_nckl": "{:.4f}".format(train_config.weight_nckl * nc_kl_loss),
